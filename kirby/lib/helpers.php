...
function css($url, $media=false, $version=false) {
  if($version) {
    $url = addTimestamp($url,'css');
  }
...
// embed a js tag
function js($url, $async = false, $version=false) {
  if($version) {
    $url = addTimestamp($url,'js');
  }
  $url   = (str::match($url, '~(^\/\/|^https?:\/\/)~'))? $url : url(ltrim($url, '/'));
  $async = ($async) ? ' async' : '';
...
function addTimestamp($url, $type) {
  $url = preg_replace('/^\//', '', $url);
  return '/'.str_replace('.'.$type, '.'.filectime($url).'.'.$type, $url);
}
